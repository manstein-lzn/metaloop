...
- `$metaloop` skill package v1.2：`skills/metaloop/SKILL.md`、`skills/metaloop/agents/openai.yaml`、`skills/metaloop/references/lightweight_protocol.md` 和 `skills/metaloop/scripts/metaloop_kernel.py` 可作为自包含 Codex Skill 部署。内置 lightweight kernel 提供 `.metaloop/mission_capsule.json`、`.metaloop/execution_report.json`、`.metaloop/verification_result.json`、status/design/run/verify/mark 的最小协议能力；design gate 会拒绝 intent-only capsule，要求 rationale、non-goal、acceptance 和硬验证路径，除非显式 `--allow-manual-only`；Mission Capsule 内锁定 `verification_spec`，当前 generic extension 支持 `file_exists`、`command`、`forbidden_path`、`json_metric_gate`，并记录 `extension_hash` 防止执行后篡改；verify 会校验 capsule/report/spec schema，并要求有效 ExecutionReport 后才可能完成。Skill 只作为入口、对齐和流程纪律，不作为不可绕过的强制层；强约束继续落在 bundled scripts/schema/validator/hook/sandbox/wrapper runtime。
