...
2. 按 Prompt Compiler 迁移顺序继续逐步接入 runtime：下一步评估 repair；主 goal prompt 暂缓。
3. 继续扩大 Co-Design v2 reviewer/brainstormer 的 domain-profile 特化检查，但不引入完整 SKS/SCP/ITC/AMP。
4. 打磨 structured resume 的 verify-only / repair-only 分支，减少不必要重跑。
5. 增加可选的 attempt commit workflow；默认仍不自动 commit。
6. 在 Codex 暴露非交互式 `/goal` API/CLI 或 thread continuation API 后替换 `CodexExecGoalRuntimeAdapter` 的传输层。
