...
- `$metaloop` skill package v1：新增 `skills/metaloop/SKILL.md`、`skills/metaloop/agents/openai.yaml`、`skills/metaloop/references/lightweight_protocol.md` 和 `skills/metaloop/scripts/metaloop_kernel.py`。Skill 可作为自包含 Codex Skill 部署；内置 lightweight kernel 提供 `.metaloop/mission_capsule.json`、`.metaloop/verification_result.json`、status/design/verify/mark 的最小协议能力。Skill 只作为入口、对齐和流程纪律，不作为不可绕过的强制层；强约束继续落在 bundled scripts/schema/validator/hook/sandbox/wrapper runtime。
