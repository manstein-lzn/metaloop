...
MetaLoop 当前是可用的本地 Alpha，但产品方向已经从“外部 CLI 编排一组 `codex exec` agent”收敛为“Codex Skill + minimal kernel + persistent Codex agent threads”。旧 v3 CLI runtime 仍可用，但复杂项目的推荐心智不再是让 MetaLoop 接管 Codex 智能，而是让 Codex thread agent 保持连续上下文，MetaLoop 负责 Mission Capsule、VerificationSpec、ExecutionReport、VerificationResult、thread registry 和审计状态。
...
产品入口层已重新定位：Codex CLI / Codex Skill 才是自然对话层，MetaLoop 不应重造一个低配聊天 TUI。第一版 Rich prompt-loop shell 和 `@openai/codex-sdk` UserAgent 可作为实验/兼容路径保留，但默认推广方向回到 `$metaloop` skill 与 bundled minimal kernel。
...
- `$metaloop` skill package v1.5：`skills/metaloop/SKILL.md`、`skills/metaloop/agents/openai.yaml`、`skills/metaloop/references/lightweight_protocol.md`、`skills/metaloop/extensions/generic/` 和 `skills/metaloop/scripts/metaloop_kernel.py` 可作为自包含 Codex Skill 部署。内置 lightweight kernel 提供 `.metaloop/mission_capsule.json`、`.metaloop/execution_report.json`、`.metaloop/verification_result.json`、`.metaloop/threads.json`、`.metaloop/event_log.jsonl`、status/design/run/verify/mark/threads/event 的最小协议能力；design gate 会拒绝 intent-only capsule；Mission Capsule 内锁定 `extension_spec` 和 `verification_spec`，generic extension 支持 executable/manual/unsupported mode 与 blocking/advisory severity，当前支持 `file_exists`、`command`、`forbidden_path`、`json_metric_gate`、`json_field_exists`、`file_contains`、`artifact_hash`、`forbidden_claim`、`manual_acceptance`、`resource_gate`；verify 会校验 capsule/report/spec hash 和 schema，manual/unsupported blocking gate 不能伪装 hard complete；`design --force` 需要 revision reason 并归档旧 capsule。Skill 只作为入口、对齐和流程纪律，不作为不可绕过的强制层；强约束继续落在 bundled scripts/schema/validator/hook/sandbox/wrapper runtime。
...
- Long-running TUI shell v1：已新增默认 `metaloop` 入口和 `metaloop shell`，启动后显示当前 workspace 的 mission/capsule/run/verification/redesign/attempt history，并允许用户持续交互。经过 StateTune/MAPE20 运行反思，这条线不再作为优先产品入口；保留为实验、兼容和本仓库 devtool。
- User-facing Agent v1：已新增 Codex SDK-backed 交互 agent，负责把“开始设计”“继续上次任务”“这次结果不满意”“现在卡在哪”这类自然语言转换为结构化 action。它可以继续作为研究样例，但团队推广默认依赖 Codex CLI 自身的 conversation + `$metaloop` skill。
- CLI 子命令继续保留：`metaloop design` / `run` / `status` / `verify` / `resume` 仍作为脚本、调试、CI 和 legacy full-repo implementation 入口。
...
完整 Python CLI 现在定位为 legacy/devtool/CI/fallback。复杂项目的默认推广路径是：在 Codex CLI 里调用 `$metaloop` skill，由持久 Codex thread agent 保持智能连续性，并用 skill-bundled kernel 写入 `.metaloop/` artifacts。
...
1. 继续强化 `$metaloop` skill 的 design/review/lock 指南，让 agent 针对任务特性设计 ExtensionSpec / VerificationSpec，而不是套模板。
2. 打磨 skill-bundled kernel 的 thread registry、event log、status 输出和 VerificationSpec validator 覆盖面。
3. 增加更多 domain extension examples，但不要把领域规则塞进 MetaLoop Core。
4. 将 RedesignProposal 应用为显式 Capsule revision 的流程做完整，包括用户确认和新 MissionSpec 生成。
5. 按 Prompt Compiler 迁移顺序继续逐步接入 runtime：下一步评估 repair；主 goal prompt 暂缓。
6. 打磨 structured resume 的 verify-only / repair-only 分支，减少不必要重跑。
7. 增加可选的 attempt commit workflow；默认仍不自动 commit。
8. 仅在明确需要 full repo CLI 时继续修 CLI/TUI；不要把模仿 Codex CLI 的 TUI v2 作为默认产品主线。
