...
2026-05-08 轻量协议层重定位后，MetaLoop 的后续方向应是 skill-first 但不是 prompt-only：`$metaloop` skill 负责入口和对齐，skill-bundled kernel/CLI/schema/validators/`.metaloop` artifacts 负责检查和状态，hooks/sandbox/wrapper runtime 在必要时提供更强约束。Skill 应能一键部署，不能要求目标环境先安装完整 MetaLoop package 才能使用核心协议。
