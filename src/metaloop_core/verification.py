...
def verify_workspace(workspace: str | Path = ".", *, write: bool = True, update_status: bool = True) -> dict[str, Any]:
    ...
    review_result, review_errors = load_review_result(root, capsule)
...
def load_review_result(workspace: str | Path, capsule: dict[str, Any] | None = None) -> tuple[dict[str, Any] | None, list[str]]:
    path = review_result_path(workspace)
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except OSError:
    ...
    errors = validate_review_result(payload, capsule)
    return (payload if isinstance(payload, dict) else None), errors


def validate_review_result(payload: Any, capsule: dict[str, Any] | None = None) -> list[str]:
    if not isinstance(payload, dict):
