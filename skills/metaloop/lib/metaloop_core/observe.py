...
def observe_node(workspace: str | Path = ".") -> dict[str, Any]:
    ...
    if (metaloop_dir / "metaloop.db").exists():
        from metaloop_core.durable import DurableStore

        return _v2_node_summary(root, DurableStore(root).observation())
...
def _v2_node_summary(root: Path, observation: dict[str, Any]) -> dict[str, Any]:
    project = observation.get("project") if isinstance(observation.get("project"), dict) else {}
    task = observation.get("selected_task") if isinstance(observation.get("selected_task"), dict) else {}
    evaluation = observation.get("latest_evaluation") if isinstance(observation.get("latest_evaluation"), dict) else {}
    event = observation.get("latest_event") if isinstance(observation.get("latest_event"), dict) else {}
    recovery_status = str(observation.get("recovery_status") or "incomplete")
    decision = str(evaluation.get("decision") or "")
    if recovery_status != "fresh":
        waiting_on = "recovery"
    elif decision == "review_required":
        waiting_on = "review"
    elif decision == "human_acceptance_required":
        waiting_on = "human_acceptance"
    elif task.get("readiness") == "blocked":
        waiting_on = "dependency"
    elif task.get("readiness") == "ready_to_accept":
        waiting_on = "acceptance"
    else:
        waiting_on = ""
    return {
        "schema": NODE_SUMMARY_SCHEMA,
        ...
        "created_at": utc_now(),
        "workspace": str(root),
        "node_id": project.get("project_id") or root.name,
        "status": observation.get("status") or "missing",
        "goal": observation.get("goal") or "",
        "current_plan": observation.get("current_plan") or "",
        "best_metric": None,
        "last_event": {
            ...
            "next_action": event.get("next_plan", ""),
        }
        if event
        else None,
        "last_verification": {
            "status": decision,
            "reason": evaluation.get("kind", ""),
            "hard_failures": 1 if decision in {"rejected", "needs_changes"} else 0,
            "manual_blockers": 1 if decision in {"review_required", "human_acceptance_required"} else 0,
            "unsupported_blockers": 1 if decision == "unsupported" else 0,
        }
        if evaluation
        else None,
        "last_review": {
            ...
            "reviewer_role": evaluation.get("authority", ""),
        }
        if evaluation.get("kind") in {"review", "human_acceptance"}
        else None,
        "adaptive_decision": event.get("decision", "") if event else "",
        "waiting_on": waiting_on,
        "outbox_count": _count_json_files(root / ".metaloop" / "outbox"),
        "inbox_count": _count_json_files(root / ".metaloop" / "inbox"),
        "pending_controls": sorted(str(item.get("type") or "") for item in pending_control_requests(root)),
        "context": {
            "state": recovery_status,
            "ready_count": 1 if recovery_status == "fresh" else 0,
            "missing": [] if recovery_status == "fresh" else ["RecoveryView"],
            "resume_brief": {"state": recovery_status},
        },
        "last_tick_action": "",
        "last_relay_status": "",
        ...
        "v2": {
            ...
            "task_state_version": task.get("state_version"),
