...
def main(argv: list[str] | None = None) -> int:
    ...
    append_event_parser.add_argument("--scope", choices=["task", "project"], default="task", help="V2 DecisionEvent scope.")
    append_event_parser.add_argument("--task", help="Explicit V2 Task subject; required for task scope.")
    append_event_parser.add_argument("--attempt", help="Optional V2 Attempt subject.")
    append_event_parser.add_argument("--evaluation", help="Optional V2 Evaluation subject.")
    append_event_parser.add_argument("--diagnosis", default="", help="Durable diagnosis for V2 recovery.")
    append_event_parser.add_argument("--supersedes", help="DecisionEvent replaced by this event.")
    ...
    list_event_parser.add_argument("--task", help="In V2, include this Task and Project event history.")
    list_event_parser.add_argument("--scope", choices=["task", "project"], help="Optional V2 scope filter.")
    list_event_parser.add_argument("--json", action="store_true", help="Print machine-readable JSON.")

    show_event_parser = event_subparsers.add_parser("show", help="Show one V2 DecisionEvent in full.")
    show_event_parser.add_argument("--event", required=True)
    ...
    v2_result = dispatch_v2(workspace, args)
    if v2_result is not None:
        return v2_result
    if (_metaloop_dir(workspace) / "metaloop.db").exists() and args.command in {
        "activate",
        "context",
        "design",
        "run",
        ...
        print(
            json.dumps(
                {
                    "error": "LegacyWriteDisabled",
                    "message": (
                        f"{args.command} uses v1 mutable artifacts and is disabled after v2 initialization; "
                        "use explicit task/attempt/evaluate/recover commands"
                    ),
                },
                indent=2,
                ensure_ascii=False,
...
def _status(workspace: Path, *, as_json: bool) -> int:
    ...
    if status.get("v2", {}).get("state") != "missing":
        print(
            f"v2: {status['v2']['state']} tasks={status['v2'].get('task_count', 0)} "
            f"selected={status['v2'].get('selected_task_id') or '-'} recovery={status['v2'].get('recovery_status') or '-'}"
        )
...
def _brief_node_summary(node: dict[str, Any]) -> dict[str, Any]:
    ...
    v2 = node.get("v2") if isinstance(node.get("v2"), dict) else {}
    return {
        ...
        "task_count": v2.get("task_count", 0),
        "selected_task_id": v2.get("selected_task_id", ""),
        "recovery_status": v2.get("recovery_status", ""),
...
def _brief_next_action(node: dict[str, Any]) -> str:
    ...
    if waiting_on == "recovery":
        return "Refresh the stale or incomplete RecoveryView before continuing."
    if waiting_on == "dependency":
        return "Complete the blocking dependency Tasks before starting another Attempt."
    if waiting_on == "acceptance":
        return "Accept the exact terminal Evaluation; do not start another Attempt."
    verification = node.get("last_verification") if isinstance(node.get("last_verification"), dict) else {}
...
def _run(workspace: Path, args: argparse.Namespace) -> int:
    ...
    report["execution_hash"] = _hash_object(report, "execution_hash")
    _metaloop_dir(workspace).mkdir(parents=True, exist_ok=True)
...
def _verify(workspace: Path, *, as_json: bool) -> int:
    ...
    review_result, review_errors = _load_review_result(workspace, capsule, execution_report)
...
def _review(workspace: Path, args: argparse.Namespace) -> int:
    ...
    execution_report, execution_errors = _load_valid_execution_report(workspace, capsule)
    if execution_report is None:
        print("No valid ExecutionReport found for review.", file=sys.stderr)
        for error in execution_errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    if args.review_command == "status":
        payload, review_errors = _load_review_result(workspace, capsule, execution_report)
        if args.json:
    ...
    if args.review_command == "record":
        ...
        review_errors = _validate_review_result(result, capsule, execution_report)
        if review_errors:
            if args.json:
...
def _observe_node(workspace: Path) -> dict[str, Any]:
    ...
    return observe_node(workspace)


def _observe_root(root: Path) -> dict[str, Any]:
    from metaloop_core.observe import observe_root

    return observe_root(root)
...
def _read_status(workspace: Path) -> dict[str, Any]:
    ...
    if isinstance(review_result, dict):
        review_errors = _validate_review_result(
            review_result,
            capsule if isinstance(capsule, dict) else None,
    ...
    v2_state: dict[str, Any] = {"state": "missing", "path": str(_metaloop_dir(workspace) / "metaloop.db")}
    if (_metaloop_dir(workspace) / "metaloop.db").exists():
        try:
            observation = DurableStore(workspace).observation()
            selected = observation.get("selected_task") if isinstance(observation.get("selected_task"), dict) else {}
            v2_state = {
                "state": "ready" if observation.get("integrity", {}).get("passed") else "invalid",
                "path": str(_metaloop_dir(workspace) / "metaloop.db"),
                "project_id": observation.get("project", {}).get("project_id"),
                "task_count": observation.get("task_count", 0),
                "selected_task_id": selected.get("task_id"),
                "selected_task_status": selected.get("readiness"),
                "recovery_status": observation.get("recovery_status"),
                "integrity": observation.get("integrity"),
            }
        except Exception as exc:
            v2_state = {"state": "invalid", "path": str(_metaloop_dir(workspace) / "metaloop.db"), "errors": [str(exc)]}
        for legacy_state in [
            capsule_state,
            adaptive_state,
            execution_state,
            ...
            if legacy_state.get("state") != "missing":
                legacy_state["state"] = "migration_input_ignored"
                legacy_state["authoritative"] = False
        if context.get("state") != "missing":
            context = {**context, "state": "migration_input_ignored", "authoritative": False}
    status = {
        "workspace": str(workspace),
...
def _next_action(status: dict[str, Any]) -> str:
    ...
    if v2.get("state") == "invalid":
        return "Run v2 integrity checks and repair the canonical store before continuing."
    if v2.get("state") == "ready":
        if v2.get("recovery_status") != "fresh":
            return "Refresh the selected Task RecoveryView before continuing."
        selected_status = v2.get("selected_task_status")
        if selected_status == "running":
            return "Continue the selected open Attempt and checkpoint durable progress."
        if selected_status == "waiting_review":
            return "Review the selected immutable Evaluation and record the authority decision."
        if selected_status == "ready_to_accept":
            return "Accept the terminal Evaluation; do not start another Attempt."
        if selected_status == "blocked":
            return "Complete the selected Task dependencies before starting another Attempt."
        if selected_status == "completed":
            return "Select another Task or report the accepted result."
        return "Start the next explicit Attempt for the selected Task."
...
def _build_review_result(
    ...
    *,
    ...
    return {
        ...
        "execution_hash": execution_report.get("execution_hash"),
        "decision": decision,
        "reviewer": reviewer,
...
def _load_review_result(
    ...
    errors = _validate_review_result(payload, capsule, execution_report)
...
def _validate_execution_report(payload: Any, capsule: dict[str, Any]) -> list[str]:
    ...
    if payload.get("execution_hash") != _hash_object(payload, "execution_hash"):
