...
- governing document、module contracts 和 migration plan 可按 hash 锁定。
- governance drift 在执行和验证前 fail closed。
...
- governance 与 v2 ContractRevision 的统一表达，但前提是存在重复消费者和迁移需求。
