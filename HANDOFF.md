...
- `review_required` 不等于用户授权。它应由独立 Codex reviewer 检查证据并通过 `review record` 写 `.metaloop/review_result.json`。MetaLoop 协议权限默认交给 Codex agents；`human_acceptance_required` 只用于用户显式保留的专属授权。
