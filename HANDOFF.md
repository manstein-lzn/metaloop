...
```text
我们继续开发 MetaLoop。仓库在 /root/metaloop。
...
当前产品方向是 Skill-only：用户通过 Codex $metaloop skill 使用 MetaLoop；仓库只保留 skills/metaloop、src/metaloop_core、当前文档和测试。不要恢复旧外部交互运行时。继续遵守 prompt-first / code-backed：prompt 负责智能，代码负责状态、验证、审计和恢复。
```
...
- Kernel 支持 `status`、`design`、`run`、`verify`、`mark`、`threads`、`event`、`adaptive`。
...
- 对开放研究任务，重点是让 design 阶段产生严格 VerificationSpec 和下一轮实验/诊断纪律。
